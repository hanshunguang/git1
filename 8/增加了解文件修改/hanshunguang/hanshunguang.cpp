...
hanshunguang::hanshunguang(QWidget *parent)
	: QWidget(parent)
{
	ui.setupUi(this);
	...
	ui.comboBox_3->addItem(tr("hanshunguang"));
	...
	m_thread->SetPath("D:/123");//扫描的路径设置模板
		...
		connect(m_thread, &mythread::signal_myzhongzhuan_1, this, &hanshunguang::signal_myzhongzhuan1);//扫描的线程与中转信号连接模板
		connect(ui.pushButton_6, &QPushButton::released, this, &hanshunguang::slot_myzhongzhuan1);//扫描的按键与中转槽连接模板
		
		connect(this, &hanshunguang::signal_myzhongzhuan1, this, &hanshunguang::slot_click1);//扫描的中转信号与处理槽连接模板
		
		//参数不同时中转-----------------------------------------------
		connect(ui.pushButton_5, &QPushButton::released, this, &hanshunguang::slot_myzhongzhuan);//参数不同时中转信号槽模板
		connect(this, &hanshunguang::signal_myzhongzhuan, this, &hanshunguang::slot_click);//参数不同时中转信号槽模板
		//参数不同时中转-----------------------------------------------
	...
	connect(ui.pushButton, &QPushButton::clicked, this, &hanshunguang::slot_btnClicked);
...
*/
void hanshunguang::slot_btnClicked(bool  Checked)
{
	ui.comboBox->addItem(tr("Circle"));
	ui.comboBox->show();
	//------------------------------------------在combox
...
hanshunguang::~hanshunguang()
{

}


void hanshunguang::slot_myzhongzhuan()//参数不同时中转信号槽模板
{
	emit hanshunguang::signal_myzhongzhuan(tr("Circle"));//参数不同时中转信号槽模板
}
void hanshunguang::slot_myzhongzhuan1()
{
	/*emit hanshunguang::signal_myzhongzhuan1(tr("Circle"));*/
	m_thread->start();
}
void hanshunguang::slot_click(QString dirlist)//参数不同时中转信号槽模板
{
	ui.textEdit->append(tr("Circle"));
}
void hanshunguang::slot_click1(QString dirlist, bool clear)//扫描的处理槽模板 参数不同时中转信号槽模板 bool clear放在信号中用于传递类和类之间的控制
{
